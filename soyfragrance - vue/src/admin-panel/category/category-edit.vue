        ...
        <div class="event-content">
            <div class="event-form">
                ...
                <div class="input-full-width">
                    <input type="text" v-model="newCategoryName" placeholder="Wprowadź nową nazwę kategorii">
                </div>
                <div class="event-button">
                    <button class="event-btn" @click="editCategoryName">Edytuj kategorię</button>
                </div>
            </div>
        </div>
    </div>
...
import axios from 'axios'
    ...
    export default {
        ...
        methods: {
            ...
            async downloadCategoryData() {
            const categoriesId = this.$route.params.id;
      try {
        const response = await axios.get(`http://localhost:8000/api/categories-data/${categoriesId}`);
        ...
        console.error('Błąd podczas pobierania danych kategorii:', error);
            ...
            async editCategoryName() {
                const categoriesId = this.$route.params.id;
      try {
        await axios.put(`http://localhost:8000/api/edit-category-name/${categoriesId}`, {
          newCategoryName: this.newCategoryName,
        });

        // Aktualizacja danych kategorii po edycji
        await this.downloadCategoryData();
        alert('Nazwa kategorii zaktualizowana pomyślnie.');
      } catch (error) {
