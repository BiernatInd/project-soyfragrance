            ...
            <table>
                ...
                <tbody>
                    <tr v-for="categories in categories" :key="categories.id">
                        <td data-label="Numer">{{ categories.id }}</td>
                        <td data-label="Nazwa kategorii">{{  categories.category_name }}</td>
                        <td data-label="Data">{{ formatData(categories.created_at)}}</td>
                        <td data-label="Szczegóły">
                            <svg width="16" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg"  @click="redirectToEditPage(categories.id)">
                            ...
                            <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg"  @click="showConfirmation(categories.id)">
...
import axios from 'axios'
    ...
    export default {
        ...
        methods: {
            fetchData() {
                // Pobierz dane z API
                axios.get('http://localhost:8000/api/categories-list')
    .then(response => {
...
formatData(isoDateTime) {
      ...
      //potwierdzenie
      showConfirmation(categoriesId) {
      console.log('showConfirmation');
      this.confirmationActive = !this.confirmationActive;
      this.selectedCategoriesId = categoriesId; // Dodaj zmienną do przechowywania ID wybranej kategorii
  ...
  deleteCategory() {
      ...
      axios
        .delete(`http://localhost:8000/api/delete-category/${this.selectedCategoriesId}`)
        .then(response => {
    ...
    //przekierowanie do edytuj
    redirectToEditPage(categoriesId) {
      // Tutaj następuje programowe przekierowanie do strony edycji
      this.$router.push(`/kategorie-edytuj/${categoriesId}`);
    },
