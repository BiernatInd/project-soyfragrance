            ...
            <table>
                <thead>
                    <tr>
                        <th>Numer</th>
                        <th>Kod rabatowy</th>
                        <th>Wartość kodu rabatowego</th>
                        <th>Ilość kodów rabatowych</th>
                        <th>Data</th>
                        <th>Szczegóły</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="promoCode in promoCode" :key="promoCode.id">
                        <td data-label="Numer">{{ promoCode.id }}</td>
                        <td data-label="Kod rabatowy">{{ promoCode.promo_code}}</td>
                        <td data-label="Wartość kodu rabatowego">{{  promoCode.discount }}%</td>
                        <td data-label="Ilość kodów rabatowych">{{  promoCode.quantity }}</td>
                        <td data-label="Data">{{ formatData(promoCode.created_at )}}</td>
                        <td data-label="Szczegóły">
                            <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg" @click="showConfirmation(promoCode.id)">
...
import axios from 'axios'   
    ...
    export default {
        ...
        methods: {
            fetchData() {
                // Pobierz dane z API
                axios.get('http://localhost:8000/api/promo-code-list')
    .then(response => {
...
formatData(isoDateTime) {
                ...
                return dateObject.toLocaleString();
                  ...
                  //potwierdzenie
      showConfirmation(promoCodeId) {
      console.log('showConfirmation');
      this.confirmationActive = !this.confirmationActive;
      this.selectedPromoCodeId = promoCodeId; // Dodaj zmienną do przechowywania ID wybranej kategorii
  ...
  hideConfirmation() {
    ...
    //usuwanie
    deletePromoCode() {
      // Usuń kategorię i ukryj potwierdzenie
      axios
        .delete(`http://localhost:8000/api/delete-promo-code/${this.selectedPromoCodeId}`)
        .then(response => {
          console.log('Kategoria została usunięta pomyślnie.', response.data);
          this.fetchData(); // Odśwież listę kategorii po usunięciu
          this.hideConfirmation();
        })
        .catch(error => {
          console.error('Błąd usuwania kategorii:', error);
          // Dodaj obsługę błędu (np. wyświetl komunikat użytkownikowi)
        });
    },
    checkUserRole() {
