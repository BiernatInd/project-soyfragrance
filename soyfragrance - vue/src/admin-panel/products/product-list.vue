            ...
            <table>
                ...
                <tbody>
                    <tr v-for="product in products" :key="product.id">
                        <td data-label="Numer">{{ product.id }}</td>
                        <td data-label="Nazwa produktu">{{ product.name }}</td>
                        <td data-label="Kategoria">{{ product.category }}</td>
                        <td data-label="Cena">{{ product.price }} zł</td>
                        <td data-label="Stan magazynowy">{{product.availability}}</td>
                        <td data-label="Szczegóły">
                            <svg width="16" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg" @click="redirectToEditPage(product.id)">
                            ...
                            <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg" @click="showConfirmation(product.id)">
        ...
        <div class="confirmation-main">
            ...
            <div class="confirmation-bottom">
                ...
                <div class="confirmation-buttons">
                    ...
                    <button class="confirmation-btn" @click="deleteProduct">Usuń</button>
                </div>
            </div>
        </div>
      </div>
...
import axios from 'axios'
    ...
    export default {
        ...
        methods: {
            fetchData() {
                // Pobierz dane z API
                axios.get('http://localhost:8000/api/products-list-admin')
                .then(response => {
      ...
      this.selectedProductId = productId; // Dodaj zmienną do przechowywania ID wybranego produktu
    ...
    hideConfirmation() {
        ...
        document.body.classList.remove('active');
            ...
            //usuwanie produktu
            deleteProduct() {
      // Usuń produkt i ukryj potwierdzenie
      axios
        .delete(`http://localhost:8000/api/delete-product/${this.selectedProductId}`)
        .then(response => {
          console.log('Produkt został usunięty:', response.data);
          this.fetchData(); // Odśwież listę produktów po usunięciu
    ...
    redirectToEditPage(productId) {
      ...
      this.$router.push(`/produkt-edytuj/${productId}`);
    },
    checkUserRole() {
