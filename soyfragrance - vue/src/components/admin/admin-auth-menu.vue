<template>
    <div class="auth-menu" :class="{ active: adminAuthMenuActive }">
...
export default {
    computed: {
    ...mapState(['adminAuthMenuActive']),
  },
  methods: {
    ...mapActions(['setAdminAuthMenuActive']),
    toggleAdminAuthMenu() {
      this.setAdminAuthMenuActive(!this.adminAuthMenuActive);
    },
    removeActiveClass() {
      this.setAdminAuthMenuActive(false);
      const adminAuthMenuElement = document.querySelector('.auth-menu');
      if (adminAuthMenuElement) {
        adminAuthMenuElement.classList.remove('active');
      }
    },
    logout() {
